...
@section('content')
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body table-responsive">
            <table class="table table-bordered table-striped {{ count($protocols) > 0 ? 'datatable' : '' }} @can('protocol_delete') @if ( request('show_deleted') != 1 ) dt-select @endif @endcan">
                <thead>
                    <tr>
                        @can('protocol_delete')
                            ...
                            <tr data-entry-id="{{ $protocol->id }}">
                                ...
                                <td>
                                    {!! Form::open(array(
                                        ...
                                        'route' => ['admin.protocols.restore', $protocol->id])) !!}
                                    ...
                                    {!! Form::close() !!}
                                        ...
                                        'method' => 'DELETE',
                                        'onsubmit' => "return confirm('".trans("global.app_are_you_sure")."');",
                                        'route' => ['admin.protocols.perma_del', $protocol->id])) !!}
                                ...
                                <td>
                                    @can('protocol_view')
                                    <a href="{{ route('admin.protocols.show',[$protocol->id]) }}" class="btn btn-xs btn-primary">@lang('global.app_view')</a>
                                    @endcan
                                    @can('protocol_edit')
                                    <a href="{{ route('admin.protocols.edit',[$protocol->id]) }}" class="btn btn-xs btn-info">@lang('global.app_edit')</a>
                                    @endcan
                                    @can('protocol_delete')
{!! Form::open(array(
                                        'style' => 'display: inline-block;',
                                        'method' => 'DELETE',
                                        'onsubmit' => "return confirm('".trans("global.app_are_you_sure")."');",
                                        'route' => ['admin.protocols.destroy', $protocol->id])) !!}
